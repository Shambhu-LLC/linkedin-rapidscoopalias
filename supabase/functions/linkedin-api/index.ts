...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      // Authenticate user - create or sign in based on LinkedIn account
      case 'authenticate-user': {
        const linkedInAccountId = body.accountId || (body as any).linkedInAccountId;
        const displayName = (body as any).displayName || 'LinkedIn User';
        const platformUserId = (body as any).platformUserId || linkedInAccountId;
        
        if (!linkedInAccountId) {
          throw new Error('Missing LinkedIn account ID');
        }

        // Create a deterministic email and password based on LinkedIn ID
        // This ensures the same LinkedIn account always maps to the same Supabase user
        const email = `linkedin_${platformUserId}@linkedin.local`;
        const password = `li_${platformUserId}_${GETLATE_API_KEY?.slice(-8) || 'secret'}`;
      ...
      // Sync LinkedIn account to database
      case 'sync-account': {
        ...
        const platformUserId = (body as any).platformUserId;
        const profilePicture = (body as any).profilePicture;
        const headline = (body as any).headline;
        const setActive = (body as any).setActive !== false;

        if (!userId || !linkedInAccountId) {
          throw new Error('Missing userId or linkedInAccountId');
        }

        // If setActive is true, deactivate all other accounts for this user
        if (setActive) {
          await supabaseAdmin
        ...
        const { data: accountData, error: accountError } = await supabaseAdmin
          ...
          .upsert({
            user_id: userId,
            linkedin_id: platformUserId || linkedInAccountId,
            getlate_account_id: linkedInAccountId,
            access_token: 'managed-by-getlate',
        ...
        if (accountError) {
          ...
          if (insertError) {
            throw new Error(`Failed to sync account: ${insertError.message}`);
          }
      ...
      // Get active account for current user
      case 'get-active-account': {
        ...
        const { data: activeAccount, error: activeError } = await supabaseAdmin
          .from('linkedin_accounts')
          .select('*')
          .eq('user_id', userId)
          .eq('is_active', true)
        ...
        if (activeError && activeError.code !== 'PGRST116') {
          throw new Error(`Failed to get active account: ${activeError.message}`);
        }
